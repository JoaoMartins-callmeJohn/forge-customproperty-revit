const { AuthClientThreeLegged, AuthClientTwoLegged, UserProfileApi } = require('forge-apis');
const { FORGE_CLIENT_ID, FORGE_CLIENT_SECRET, FORGE_CALLBACK_URL, INTERNAL_TOKEN_SCOPES, PUBLIC_TOKEN_SCOPES, INTERNAL_DA_TOKEN_SCOPES } = require('../../config.js');
...
const internalDAAuthClient = new AuthClientTwoLegged(FORGE_CLIENT_ID, FORGE_CLIENT_SECRET, INTERNAL_DA_TOKEN_SCOPES, true);
...
async function getInternalDAToken() {
    if (!internalDAAuthClient.isAuthorized()) {
        await internalDAAuthClient.authenticate();
    }
    return internalDAAuthClient.getCredentials();
}
